    ...
    <div className="border border-zinc-200 bg-white p-5 md:p-6 xl:p-8">
      ...
      <div className="space-y-2">
        {(block.options || []).map((option, index) => {
          ...
          return (
            <button key={index} type="button" onClick={() => { if (submitted) return; setSelected((current) => multi ? current.includes(option) ? current.filter((item) => item !== option) : [...current, option] : [option]); }} className={active ? 'w-full rounded-2xl border border-zinc-900 bg-zinc-900 px-4 py-3 text-left text-sm text-white' : 'w-full rounded-2xl border border-zinc-200 px-4 py-3 text-left text-sm text-zinc-700 transition hover:border-zinc-400 hover:bg-zinc-50'}>{option}</button>
          );
        })}
      </div>
      <div className="mt-4 flex items-center justify-between gap-3">
        <div className="text-xs uppercase tracking-[0.18em] text-zinc-500">{multi ? 'Multi-answer' : 'Single answer'}</div>
        <button type="button" onClick={submit} disabled={selected.length === 0} className="rounded-2xl border border-zinc-900 bg-zinc-900 px-4 py-2 text-sm font-medium text-white transition hover:bg-zinc-800 disabled:opacity-40">Check</button>
      </div>
    </div>
  );
}
    ...
    <div className="border border-zinc-200 bg-white p-5 md:p-6 xl:p-8">
      ...
      <div className="space-y-3">
        {values.map((value, index) => (
          <textarea key={index} rows={block.taskType === 'open' ? 5 : 2} value={value} onChange={(event) => setValues((current) => current.map((entry, currentIndex) => currentIndex === index ? event.target.value : entry))} className="w-full resize-y rounded-2xl border border-zinc-200 px-4 py-3 text-sm outline-none transition focus:border-zinc-900" />
        ))}
      </div>
      <div className="mt-5 flex items-center justify-between gap-3">
        <div className="text-xs text-zinc-500">{block.hint || 'Type your answer.'}</div>
        <button type="button" onClick={submit} disabled={values.every((value) => !value.trim())} className="rounded-2xl border border-zinc-900 bg-zinc-900 px-4 py-2 text-sm font-medium text-white transition hover:bg-zinc-800 disabled:opacity-40">{block.taskType === 'open' ? 'Save' : 'Check'}</button>
      </div>
      {submitted && (() => {
        const pct = Math.round(values.reduce((total, value, index) => total + similarity(value, answers[index] || ''), 0) / Math.max(values.length, 1) * 100);
        return <div className={['mt-4 rounded-2xl border px-4 py-3 text-sm', pct >= 85 ? 'border-emerald-300 bg-emerald-50 text-emerald-800' : pct >= 50 ? 'border-amber-300 bg-amber-50 text-amber-800' : 'border-red-300 bg-red-50 text-red-800'].join(' ')}>{pct >= 85 ? 'Great match!' : pct >= 50 ? 'Close — check your answer.' : 'Review the expected answer.'} Score: {pct}%{pct < 85 && answers[0] && <span className="ml-2">Expected: <strong>{answers.join(', ')}</strong></span>}</div>;
      })()}
    </div>
  );
}
...
function GridRenderer({ block, onComplete }) {
  ...
  return (
    <div className="border border-zinc-200 bg-white p-5 md:p-6 xl:p-8">
      ...
      <button type="button" onClick={() => onComplete?.({ submitted: true, correct: true, score: 1, response: values, feedback: 'Grid saved.' })} className="mt-5 rounded-2xl border border-zinc-900 bg-zinc-900 px-4 py-2 text-sm font-medium text-white transition hover:bg-zinc-800">Save grid</button>
    </div>
  );
}
  ...
  return (
    <div className="border border-zinc-200 bg-white p-5 md:p-6 xl:p-8">
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_280px]">
        ...
        <div className="space-y-3">
          <textarea rows={8} placeholder="Add notes, labels, answers, or observations" className="w-full resize-y rounded-2xl border border-zinc-200 px-4 py-3 text-sm outline-none transition focus:border-zinc-900" />
          <button type="button" onClick={() => onComplete?.({ submitted: true, correct: true, score: 1, response: 'saved', feedback: 'Media response saved.' })} className="rounded-2xl border border-zinc-900 bg-zinc-900 px-4 py-2 text-sm font-medium text-white transition hover:bg-zinc-800">Save response</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="border border-zinc-200 bg-white p-5 md:p-6 xl:p-8">
      ...
      {hasContent ? (
        <div className="grid gap-4 lg:grid-cols-3">
          {block.items?.length > 0 && <div className="rounded-2xl border border-zinc-200 p-4 text-sm text-zinc-700"><div className="mb-2 text-xs font-medium uppercase tracking-[0.18em] text-zinc-500">Items</div>{block.items.map((item, index) => <div key={index} className="py-1">{item}</div>)}</div>}
          {block.pairs?.length > 0 && <div className="rounded-2xl border border-zinc-200 p-4 text-sm text-zinc-700"><div className="mb-2 text-xs font-medium uppercase tracking-[0.18em] text-zinc-500">Pairs</div>{block.pairs.map((pair, index) => <div key={index} className="py-1">{pair.left} &rarr; {pair.right}</div>)}</div>}
          {block.categories?.length > 0 && <div className="rounded-2xl border border-zinc-200 p-4 text-sm text-zinc-700"><div className="mb-2 text-xs font-medium uppercase tracking-[0.18em] text-zinc-500">Categories</div>{block.categories.map((category, index) => <div key={index} className="py-1">{category}</div>)}</div>}
          {block.options?.length > 0 && !block.items?.length && <div className="rounded-2xl border border-zinc-200 p-4 text-sm text-zinc-700"><div className="mb-2 text-xs font-medium uppercase tracking-[0.18em] text-zinc-500">Options</div>{block.options.map((option, index) => <div key={index} className="py-1">{option}</div>)}</div>}
        </div>
      ) : (
        <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">This task has no content to display. Check Items, Options, or Pairs in the DSL.</div>
      )}
      <button type="button" onClick={() => onComplete?.({ submitted: true, correct: true, score: 1, response: 'completed', feedback: block.explanation || block.hint || '' })} className="mt-5 rounded-2xl border border-zinc-900 bg-zinc-900 px-4 py-2 text-sm font-medium text-white transition hover:bg-zinc-800">Mark complete</button>
    </div>
  );
}

export default function GenericTask({ block, onComplete, existingResult }) {
  const definition = getTaskDefinition(block.taskType);
