...
export default function CollectionTask({ block, onComplete }) {
    ...
    <div className="border border-zinc-200 bg-white p-5 md:p-6 xl:p-8">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        {items.map((item, index) => (
          <label key={index} className={checked.includes(item) ? 'flex items-start gap-3 border border-zinc-900 bg-zinc-50 px-4 py-4 text-sm text-zinc-900 transition' : 'flex items-start gap-3 border border-zinc-200 bg-white px-4 py-4 text-sm text-zinc-700 transition hover:border-zinc-400'}>
      ...
      {block.categories?.length > 0 && (
        <div className="mt-4 border border-zinc-200 p-3 text-sm text-zinc-700">
      ...
      <div className="mt-5 flex items-center justify-between gap-3">
        ...
        <button type="button" onClick={() => onComplete?.({ submitted: true, correct: true, score: Math.min(1, checked.length / Math.max(items.length, 1)), response: checked, feedback: block.explanation || block.hint || 'Collection saved.' })} disabled={checked.length === 0} className="border border-zinc-900 bg-zinc-900 px-4 py-2 text-sm font-medium text-white transition hover:bg-zinc-800 disabled:opacity-40">Save set</button>
      </div>
    </div>
  );
}
