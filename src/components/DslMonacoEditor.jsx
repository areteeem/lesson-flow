...
const FIELD_KEYS = [
  ...
  'FocusWords', 'HideMode', 'HideCount', 'HideMinLength', 'RequireAll', 'FlexibleOrder',
];
...
const FIELD_DOCS = {
  ...
  columns: 'Column headers for table/grid tasks.',
  rows: 'Row data for table/grid tasks. Use | separator for cells.',
  items: 'Ordered list of items for sequence, wheel, or vocabulary tasks.',
  cards: 'Card definitions with front/back for flashcard tasks.',
  hint: 'Optional hint shown to the learner before answering.',
  explanation: 'Explanation shown after the learner submits their answer.',
  multiple: 'When true, allows selecting multiple answers.',
  shuffle: 'When true, randomizes the order of options or items.',
  layout: 'Layout hint for rendering: grid, list, or inline.',
  focuswords: 'Target words to hide in word-hide task variants.',
  hidemode: 'reveal | drag | type — how hidden words are restored.',
  hidecount: 'Number of additional random words to hide beyond focusWords.',
  hideminlength: 'Minimum word length for random hiding.',
  requireall: 'When true, all rows must be answered for grid_select tasks.',
  flexibleorder: 'When true, accepts shifted fill answers across consecutive blanks.',
      ...
      if (!lineContent.trim() || /^[A-Za-z]*$/.test(lineContent.trimStart())) {
        FIELD_KEYS.forEach((key) => {
          suggestions.push({
            ...
            insertText: `${key}: `,
            range,
            documentation: FIELD_DOCS[key.toLowerCase()] || `DSL field: ${key}`,
          });
        });
      }
  ...
  monaco.languages.registerCompletionItemProvider(DSL_LANGUAGE_ID, {
    ...
    provideCompletionItems(model, position) {
      ...
      const snippets = {
        ...
        GRID_SELECT: '#TASK: GRID_SELECT\nQuestion: ${1:Rate each statement.}\nColumns:\n${2:Agree}\n${3:Neutral}\n${4:Disagree}\nRows:\n${5:I enjoy reading.}\n${6:I prefer group work.}\nPairs:\n${7:I enjoy reading.} => ${8:Agree}\n${9:I prefer group work.} => ${10:Neutral}\n',
        WORD_HIDE_REVEAL: '#TASK: WORD_HIDE_REVEAL\nQuestion: ${1:Read and reveal the hidden words.}\nText: ${2:The quick brown fox jumps over the lazy dog.}\nFocusWords:\n${3:quick}\n${4:jumps}\n${5:lazy}\nHideMode: reveal\nHideCount: 2\nHideMinLength: 3\n',
        WORD_HIDE_DRAG: '#TASK: WORD_HIDE_DRAG\nQuestion: ${1:Drag the missing words back.}\nText: ${2:The quick brown fox jumps over the lazy dog.}\nFocusWords:\n${3:quick}\n${4:jumps}\nHideMode: drag\nHideCount: 2\nHideMinLength: 3\n',
        HIGHLIGHT_MISTAKE: '#TASK: HIGHLIGHT_MISTAKE\nQuestion: ${1:Highlight the mistake.}\nText: ${2:She go to school every day.}\nAnswer: ${3:go}\nInstruction: Tap the word that is incorrect.\n',
        SELECT_AND_CORRECT: '#TASK: SELECT_AND_CORRECT\nQuestion: ${1:Select and correct the error.}\nText: ${2:He don\'t like apples.}\nAnswer: ${3:doesn\'t}\nInstruction: Tap the wrong word, then type the correction.\n',
        FILL_TABLE_MATRIX: '#TASK: FILL_TABLE_MATRIX\nQuestion: ${1:Fill the table.}\nColumns:\n${2:Subject}\n${3:Verb}\nRows:\n${4:I / You | work}\n${5:He / She | works}\n',
        TABLE_REVEAL: '#TASK: TABLE_REVEAL\nQuestion: ${1:Reveal the hidden values.}\nColumns:\n${2:Subject}\n${3:Verb}\nRows:\n${4:I / You | work}\n${5:He / She | works}\nRevealMode: manual\nHiddenCells:\n${6:0:1}\n${7:1:1}\n',
