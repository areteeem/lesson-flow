...
export const TASK_REGISTRY = [
  ...
  defineTask('true_false', 'True / False', 'choice', 'Core Practice', ['true', 'false', 'binary'], 'Decide whether a statement is correct.', { hiddenFromLibrary: true, mergedInto: 'multiple_choice' }),
  defineTask('yes_no', 'Yes / No', 'choice', 'Core Practice', ['yes', 'no', 'binary'], 'Quick binary response for checks and opinion prompts.', { hiddenFromLibrary: true, mergedInto: 'multiple_choice' }),
  defineTask('either_or', 'Either / Or', 'choice', 'Core Practice', ['choice', 'binary', 'compare'], 'Force a choice between two alternatives.', { hiddenFromLibrary: true, mergedInto: 'multiple_choice' }),
  ...
  defineTask('type_in_blank', 'Type In Blank', 'text', 'Input', ['type', 'gap fill', 'blank', 'typing'], 'Type answers directly into blank spaces in a sentence.', { hiddenFromLibrary: true, mergedInto: 'fill_typing' }),
  ...
  defineTask('drag_drop', 'Drag And Drop', 'pairs', 'Matching', ['drag', 'drop', 'sort'], 'Drag items into matching targets.', { hiddenFromLibrary: true, mergedInto: 'match' }),
  ...
  defineTask('timeline_order', 'Timeline Order', 'collection', 'Sequencing', ['timeline', 'order', 'events'], 'Arrange events in chronological order.', { hiddenFromLibrary: true, mergedInto: 'order' }),
  defineTask('sentence_builder', 'Sentence Builder', 'collection', 'Writing', ['sentence', 'builder', 'word order'], 'Assemble a sentence from fragments.', { hiddenFromLibrary: true, mergedInto: 'order' }),
  ...
  defineTask('compare_contrast_table', 'Compare Contrast Table', 'grid', 'Data Entry', ['compare', 'contrast', 'table'], 'Fill a table comparing two or more items.', { hiddenFromLibrary: true, mergedInto: 'fill_table_matrix' }),
  defineTask('map_geography_label', 'Map Geography Label', 'media', 'Visual', ['map', 'geography', 'label'], 'Place labels onto a map or diagram.', { hiddenFromLibrary: true, mergedInto: 'image_labeling' }),
  ...
  defineTask('story_reconstruction', 'Story Reconstruction', 'collection', 'Sequencing', ['story', 'sequence', 'reconstruction'], 'Rebuild a narrative from mixed parts.', { hiddenFromLibrary: true, mergedInto: 'order' }),
  ...
  defineTask('categorize_grammar', 'Categorize Grammar', 'collection', 'Grammar', ['grammar', 'sort', 'categorize'], 'Sort examples by grammar rule or pattern.', { hiddenFromLibrary: true, mergedInto: 'categorize' }),
  ...
  defineTask('drag_match', 'Drag Match', 'pairs', 'Matching', ['drag', 'match', 'drop', 'definitions'], 'Drag items onto their matching definitions or targets.', { hiddenFromLibrary: true, mergedInto: 'match' }),
  defineTask('table_drag', 'Table Drag', 'grid', 'Data Entry', ['table', 'drag', 'cells', 'grid'], 'Drag answer options into the correct table cells.', { hiddenFromLibrary: true, mergedInto: 'fill_table_matrix' }),
  ...
  defineTask('dialogue_reconstruct', 'Dialogue Reconstruct', 'collection', 'Sequencing', ['dialogue', 'reconstruct', 'order', 'conversation'], 'Reorder scrambled dialogue messages into the correct sequence.', { hiddenFromLibrary: true, mergedInto: 'order' }),
...
const LEGACY_TASK_MAP = {
  ...
  type_in_blank: 'fill_typing',
  true_false: 'multiple_choice',
  yes_no: 'multiple_choice',
  either_or: 'multiple_choice',
  drag_drop: 'match',
  drag_match: 'match',
  timeline_order: 'order',
  sentence_builder: 'order',
  story_reconstruction: 'order',
  dialogue_reconstruct: 'order',
  categorize_grammar: 'categorize',
  compare_contrast_table: 'fill_table_matrix',
  table_drag: 'fill_table_matrix',
  map_geography_label: 'image_labeling',
};

export function resolveCanonicalTaskType(taskType) {
  return LEGACY_TASK_MAP[taskType] || taskType;
}

export function getTaskDefinition(taskType) {
  const canonical = resolveCanonicalTaskType(taskType);
  return TASK_TYPE_MAP[canonical] || { type: canonical, label: canonical, kind: 'generic', category: 'Other', keywords: [], description: '' };
}
